...
int OpenOutPut()
{
	...
	if(pFormatCtx_Audio != NULL &&pFormatCtx_Audio->streams[0]->codec->codec_type == AVMEDIA_TYPE_AUDIO)
	{
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	while(1)
	{
		...
		if (fifo_audio && fifo_video)
		{
			...
			//缓存数据写完就结束循环
			if (av_audio_fifo_size(fifo_audio) <= pFormatCtx_Out->streams[AudioIndex]->codec->frame_size && av_fifo_size(fifo_video) <= frame_size && !bCap)
			{
				break;
			}
		}
		...
		if(av_compare_ts(cur_pts_v, pFormatCtx_Out->streams[VideoIndex]->time_base,cur_pts_a,pFormatCtx_Out->streams[AudioIndex]->time_base) <= 0)
		{
			...
			if(av_fifo_size(fifo_video) >= size)
			{
				...
				avpicture_fill((AVPicture *)picture, picture_buf,pFormatCtx_Out->streams[VideoIndex]->codec->pix_fmt, 
					pFormatCtx_Out->streams[VideoIndex]->codec->width, pFormatCtx_Out->streams[VideoIndex]->codec->height);
...
DWORD WINAPI ScreenCapThreadProc( LPVOID lpParam )
{
	...
	while(bCap)
	{
		...
		if(packet->stream_index == 0)
		{
			...
			if (got_picture)
			{
				sws_scale(img_convert_ctx, (const uint8_t* const*)pFrame->data,pFrame->linesize,0,pFormatCtx_Out->streams[VideoIndex]->codec->height, picture->data, picture->linesize);
